...
@pytest.mark.parametrize(
    ("feature_enabled", "expected_status"), ((False, None), (True, "open"))
)
def test_talk_view_signup_status_gated_by_feature_flag(
...
@pytest.mark.parametrize(
    ("feature_enabled", "user_email", "event_settings", "expected"),
    (
        (False, "ok@example.com", {}, False),
        (True, "ok@example.com", {}, True),
        (
            True,
            "user@forbidden.example",
            {"signup_domains": ["allowed.example"]},
            False,
        ),
    ),
)
def test_talk_view_signup_allowed_for_user(
    feature_enabled, user_email, event_settings, expected
):
    event = EventFactory(
        feature_flags={"attendee_signup": feature_enabled},
        attendee_signup_settings=event_settings,
    )
    sub_type = SubmissionTypeFactory(event=event, attendee_signup_required=True)
    submission = SubmissionFactory(
        event=event,
    ...
    with scope(event=event):
        assert view.signup_allowed_for_user is expected
