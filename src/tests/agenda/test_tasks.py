...
def test_export_schedule_html_nonexistent_event():
    result = task_export_schedule_html(event_id=99999)

    assert result is None


def test_export_schedule_html_no_schedule(event):
    result = task_export_schedule_html(event_id=event.pk)
...
def test_export_schedule_html_without_cached_file(published_talk_slot):
    result = task_export_schedule_html(event_id=published_talk_slot.submission.event.pk)
