...
def test_event_on_custom_domain(event_on_foobar, client):
    r = client.get('/{}/'.format(event_on_foobar.slug), HTTP_HOST='foobar')
    assert r.status_code == 200


@pytest.mark.django_db
def test_event_on_custom_port(event_on_custom_port, client):
    r = client.get('/{}/'.format(event_on_custom_port.slug), HTTP_HOST='foobar:8000')
    assert r.status_code == 200


@pytest.mark.django_db
def test_event_on_custom_domain_in_orga_area(event_on_foobar, client):
    r = client.get('/orga/event/{}/'.format(event_on_foobar.slug), HTTP_HOST='foobar')
    assert r.status_code == 302
    assert r['Location'] == 'http://example.com/orga/event/{}/'.format(event_on_foobar.slug)


@pytest.mark.django_db
def test_event_on_custom_port_in_orga_area(event_on_custom_port, client):
    r = client.get('/orga/event/{}/'.format(event_on_custom_port.slug), HTTP_HOST='foobar:8000')
    assert r.status_code == 302
    assert r['Location'] == 'http://example.com/orga/event/{}/'.format(event_on_custom_port.slug)


@pytest.mark.django_db
def test_event_with_custom_domain_on_main_domain(event_on_foobar, client):
    """ redirect from common domain to custom domain """
    r = client.get('/{}/'.format(event_on_foobar.slug), HTTP_HOST='example.com')
    assert r.status_code == 302
    assert r['Location'] == 'https://foobar/{}/'.format(event_on_foobar.slug)


@pytest.mark.django_db
def test_event_with_custom_port_on_main_domain(event_on_custom_port, client):
    """ redirect from common domain to custom domain """
    r = client.get('/{}/'.format(event_on_custom_port.slug), HTTP_HOST='example.com')
    assert r.status_code == 302
    assert r['Location'] == 'https://foobar:8000/{}/'.format(event_on_custom_port.slug)
...
def test_cookie_domain_on_custom_domain(event_on_foobar, client):
    r = client.get('/{}/login/'.format(event_on_foobar.slug), HTTP_HOST='foobar')
    assert r.status_code == 200
...
def test_cookie_domain_on_main_domain(event, client):
    with override_settings(SESSION_COOKIE_DOMAIN='example.com'):
        r = client.get('/{}/login/'.format(event.slug), HTTP_HOST='example.com')
        assert r.status_code == 200
...
def test_with_forwarded_host(event_on_foobar, client):
    ...
    r = client.get('/{}/'.format(event_on_foobar.slug), HTTP_X_FORWARDED_HOST='foobar')
    assert r.status_code == 200
