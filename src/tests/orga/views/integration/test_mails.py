...
def test_mail_detail_view_pending(client, event, draft_mail):
    ...
    response = client.get(draft_mail.urls.base)
...
def test_delete_confirm_page(client, event, draft_mail):
    ...
    response = client.get(draft_mail.urls.delete, follow=True)
...
def test_template_list_view(
    ...
    with scopes_disabled():
        for _ in range(item_count - 1):
            MailTemplateFactory(event=event)
...
def test_compose_session_mail_get(client, event, submission):
    ...
    response = client.get(event.orga_urls.compose_mails_sessions, follow=True)
...
def test_compose_teams_mail_get(client, event):
    ...
    response = client.get(event.orga_urls.compose_mails_teams, follow=True)
...
def test_compose_mail_choice_get(client, event):
    ...
    url = reverse("orga:mails.compose.choose", kwargs={"event": event.slug})
    response = client.get(url)
