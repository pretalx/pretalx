...
@pytest.mark.django_db
def test_invite_speaker_uses_provided_name(submission):
    with scopes_disabled():
        user = User.objects.create_user(
            email="newname@example.com", password="pw!", name="User Name"
        )
    with scope(event=submission.event):
        submission.invite_speaker(email=user.email, name="Organiser Name")
        profile = SpeakerProfile.objects.get(user=user, event=submission.event)
        assert profile.name == "Organiser Name"
