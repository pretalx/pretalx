...
from tests.factories import QuestionFactory, SubmissionFactory, UserFactory
...
def test_questions_step_applicable_with_questions(cfp_event):
...
def test_info_step_dedraft_submission_returns_draft(cfp_event, cfp_user):
    ...
    profile, _ = SpeakerProfile.objects.get_or_create(user=cfp_user, event=cfp_event)
...
def test_info_step_dedraft_submission_none_for_wrong_user(cfp_event):
    ...
    profile, _ = SpeakerProfile.objects.get_or_create(user=other_user, event=cfp_event)
    draft.speakers.add(profile)
