...
def test_answer_filter_filters_by_question(event):
    ...
    fs = AnswerFilterSet(data={"question": str(q1.pk)}, queryset=Answer.objects.all())
...
def test_answer_filter_filters_by_submission_code(event):
    ...
    fs = AnswerFilterSet(data={"submission": sub1.code}, queryset=Answer.objects.all())
...
def test_answer_filter_filters_by_speaker_code(event):
    ...
    fs = AnswerFilterSet(data={"speaker": speaker.code}, queryset=Answer.objects.all())
