...
def test_questions_form_init_filters_by_active_flag(active, should_have_field):
    event = EventFactory()
    q = QuestionFactory(event=event, target=QuestionTarget.SUBMISSION, active=active)
...
def test_questions_form_get_object_for_question_speaker():
    ...
    form = QuestionsForm(event=event, speaker=speaker, target=QuestionTarget.SPEAKER)
