...
document.querySelector("button#generate-widget").addEventListener("click", (_event) => {
    ...
    const days = Array.from(
        document.querySelector("#id_days").querySelectorAll("option:checked"),
        (e) => e.value,
    )
    const rooms = Array.from(
        document.querySelector("#id_rooms").querySelectorAll("option:checked"),
        (e) => e.value,
    )
    let dataFilter = ""
    if (days.length) {
