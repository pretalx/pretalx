...
;(() => {
    ...
    class Formset {
        ...
        getEmptyForm(type) {
            if (type) {
                return this.formset.querySelector(
                    `${this.opts.emptyForm}[data-formset-empty-form="${type}"]`,
                )
            }
        ...
        addForm(type) {
            ...
            if (this.opts.animateForms) {
                Array.from(temp.children).forEach((child) => {
                    if (child.matches?.(this.opts.form)) {
        ...
        bindForm(form, index) {
            ...
            const onChangeDelete = () => {
                if (deleteCheckbox.checked) {
                    ...
                    form.dispatchEvent(
                        new CustomEvent("formDeleted", { bubbles: true }),
                    )
                } else {
                    ...
                    form.querySelectorAll(
                        '[data-formset-required-field="true"]',
                    ).forEach((field) => {
                        field.required = true
                    })
                    form.querySelectorAll("[data-formset-field-pattern]").forEach(
                        (field) => {
        ...
        slideDown(element) {
            ...
            element.style.transition =
                "height 0.4s ease, padding 0.4s ease, margin 0.4s ease"
        ...
        getActiveForms() {
            return this.getForms().filter(
                (form) => !("formsetFormDeleted" in form.dataset),
            )
        }
