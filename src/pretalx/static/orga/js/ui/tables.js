...
const refreshTableContent = (tableName, url) => {
...
const setupColumnPicker = (root) => {
    ...
    pool.querySelectorAll(".column-chip").forEach((chip) =>
    ...
    const getColumns = () => Array.from(shown.children).map((row) => row.dataset.column)
...
const setupPrintButton = (button) => {
    ...
    button.addEventListener("click", () => printCurrentTable(button.dataset.tableName))
}
