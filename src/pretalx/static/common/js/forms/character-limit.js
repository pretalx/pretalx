...
onReady(() => {
    ...
    const excessIndex = (normalized, countIn, max) => {
        if (countIn !== "words") {
            ...
            for (
                let counted = 0;
                counted < max && index < normalized.length;
                counted += 1
            ) {
    ...
    const updateHighlight = (element, normalized, index) => {
        ...
        const highlightClass = isTextarea
        ...
        highlightDiv.innerHTML =
            ...
            '<mark class="character-limit-excess">' +
            escapeHtml(normalized.substring(index)) +
            "</mark>"
    ...
    const validateField = (element) => {
        ...
        if (element.tagName === "TEXTAREA" || element.tagName === "INPUT") {
            updateHighlight(
                element,
                normalized,
                isOverLimit ? excessIndex(normalized, countIn, maxLength) : -1,
            )
        }
    ...
    document
        .querySelectorAll("[data-maxlength], [data-minlength]")
        .forEach(initializeField)
})
