...
const updateColAllCheckbox = (permission) => {
    const checkboxes = document.querySelectorAll(
        `#endpoint-permissions-table tbody input[type="checkbox"][data-permission="${permission}"]`,
    )
    const colAllCheckbox = document.querySelector(
        `.permission-col-all[data-permission="${permission}"]`,
    )
    const allChecked = Array.from(checkboxes).every((cb) => cb.checked)
...
const setupPermissionCheckboxes = () => {
    ...
    table
        .querySelectorAll('tbody input[type="checkbox"][data-permission]')
        .forEach((cb) => {
