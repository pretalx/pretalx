...
const validateSelect = (element, addErrors = false) => {
    ...
    if (isRequired && !hasValue) {
        if (addErrors) {
            ...
            if (
                !container.nextElementSibling?.classList.contains("js-validation-error")
            ) {
                const feedback = document.createElement("div")
                feedback.className = "invalid-feedback js-validation-error"
                feedback.textContent =
                    element.dataset.requiredMessage || "Please select an option."
...
const initSelect = (element) => {
    ...
    const choicesOptions = {
        ...
        removeItemButton: !element.readonly && (!isRequired || element.multiple),
    ...
    choicesOptions.callbackOnCreateTemplates = (
        _strToEl,
        _escapeForTemplates,
        _getClassNames,
    ) => ({
        choice: (allowHTML, choice, _unused, selectedText, groupName) => {
            const originalResult = Choices.defaults.templates.choice(
                allowHTML,
                choice,
                _unused,
                selectedText,
            ...
            if (
                choice.element?.dataset.description &&
                choice.element.dataset.description.length > 0
            ) {
            ...
            if (
                choice.element?.dataset.color &&
                choice.element.dataset.color.length > 0
            ) {
            ...
            if (choice.element?.dataset.fontFamily) {
        ...
        item: (_a, choice, removeItemButton) => {
            const originalResult = Choices.defaults.templates.item(
                _a,
                choice,
                removeItemButton,
            )
    ...
    document.querySelectorAll("form").forEach((form) => {
        ...
        form.querySelectorAll(
            ...
            button.addEventListener("click", (e) => {
                ...
                form.querySelectorAll('select.enhanced[data-required="true"]').forEach(
                    (select) => {
                        if (select.closest("[data-formset-form-deleted]")) return
                        if (!validateSelect(select, true) && !firstInvalid)
                            firstInvalid = select
                    },
                )
                if (firstInvalid) {
