...
- :feature:`admin` Released pretalx packages (wheels on PyPI) now ship prebuilt frontend code, so installing pretalx from PyPI no longer requires Node.js/npm to be present on the server. Building or installing from source (including from an sdist/tarball) still rebuilds the frontend and requires npm as before.
